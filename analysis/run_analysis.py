...
def make_evaluation_plots(data, hue, base_dir):
    # data = data[(data['Test'] != 182)]

    fig, ax = plt.subplots(1, 2, figsize=(10,3.5))
    sns.barplot(data= data, x='Interval', y='MSE_ts', hue=hue, palette="viridis", ax=ax[0], errorbar = 'se')
    sns.barplot(data= data, x='Interval', y='PearsonR_ts', hue=hue, palette="viridis", ax=ax[1], errorbar = 'se')
    ax[0].set_ylim(bottom=0)
    # ax[1].set_ylim(0, 100)
    ax[1].set_ylim(0, 1)
    fig.tight_layout()
    fig.savefig(f"{base_dir}/Comparison_Plot_ts.png", dpi=300)
    plt.close()

    fig, ax = plt.subplots(1, 2, figsize=(10,3.5))
    sns.barplot(data= data, x='Interval', y='MSE_0', hue=hue, palette="viridis", ax=ax[0], errorbar = 'se')
    sns.barplot(data= data, x='Interval', y='PearsonR_0', hue=hue, palette="viridis", ax=ax[1], errorbar = 'se')
    ax[0].set_ylim(bottom=0)
    # ax[1].set_ylim(0, 100)
    ax[1].set_ylim(0, 1)
    fig.tight_layout()
    ...
    sns.barplot(data=data, x="Interval", y="Time_To_Train", hue=hue, palette="viridis", ax=ax, errorbar = 'se')
...
def main(task:str, model_types:list[str], configs:list[str], time_shift:int=10):
    ...
    make_evaluation_plots(data, 'Model', base_dir)
    ...
    data = pd.read_csv(f"../{task}/{time_shift}/model_evaluation/best_combinations.csv", index_col=False)
    zoom_df = pd.read_csv("../label/correct_metrics_time_window.csv", index_col=False)

    output_file_dir = f"../{task}/{time_shift}/output_df/default"
