...
data1 = np.load("./output_images32/cache1.npz")
data2 = np.load("./output_images32/cache2.npz")
...
ax.text(-0.17, 0.5, "Amplitude [mm/s]", transform=ax.transAxes,
        rotation="vertical", va="center", ha="left",fontweight="bold",
        fontsize=7,
    )
ax.xaxis_date()
...
# ax.set_ylabel("Amplitude [mm/s]", fontsize=7, weight="bold")
ax.text(-0.17, 0.5, "Impact Force [kN]", transform=ax.transAxes,
        rotation="vertical", va="center", ha="left",fontweight="bold",
        fontsize=7,
    )
ax.set_xlabel(f"Time [UTC+0]", fontweight='bold', fontsize=7)
ax.xaxis_date()
ax.xaxis.set_major_formatter(mdates.DateFormatter('%m-%d\n%H:%M'))
ax.xaxis.set_major_locator(mdates.MinuteLocator(interval=15))
ax.set_ylim(0, 40)
...
ax.plot([UTCDateTime(i).matplotlib_date for i in data1['times1']], data1['force1'], label="Raw IF Data", color="blue", linewidth=1)
ax.plot([UTCDateTime(i).matplotlib_date for i in data1['times2']], data1['force2'], label="Shifted+\nSynthetic IF Data", color="k", linewidth=1, alpha=0.8)
ax.plot([UTCDateTime(i).matplotlib_date for i in data1['times3']], data1['force3'], label="Trimed+\nSmoothened IF Data", color="r", linewidth=1.5, alpha=0.8)
...
ax.plot([UTCDateTime(i).matplotlib_date for i in data2['times1']], data2['force1'], label="Raw IF Data", color="blue", linewidth=1)
ax.plot([UTCDateTime(i).matplotlib_date for i in data2['times2']], data2['force2'], label="Shifted+\nSynthetic IF Data", color="k", linewidth=1, alpha=0.8)
ax.plot([UTCDateTime(i).matplotlib_date for i in data2['times3']], data2['force3'], label="Trimed+\nSmoothened IF Data", color="r", linewidth=1.5, alpha=0.8)
...
plt.tight_layout()
png_path = Path(current_dir) / f"Figure-2.png"
