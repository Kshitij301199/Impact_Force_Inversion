...
gs = gridspec.GridSpec(3, 3, width_ratios=[10, 10, 0.5])

subplot_idx = ["(a)", "(b)", "(c)", "(d)", "(e)", "(f)"]
cmap = "Oranges"
ax_list = {
    ...
    4: plt.subplot(gs[2, 0]),
    5: plt.subplot(gs[2, 1]),
    6: plt.subplot(gs[0, 2]),
    7: plt.subplot(gs[1, 2]),
    8: plt.subplot(gs[2, 2]),
}


idx = 0
metric = "Peak_Diff_percent"
for model in ["xLSTM", "LSTM"]:
    for time_shift in ["0", "1", "average"]:  # ["0", "average", "1"]:
        ax = ax_list.get(idx)

        result_dir = "../../output_final3"
        result_dirs = [f"{result_dir}/task2_5_30_{i}" for i in [1, 2]]
        main_df = None
        for result_dir in result_dirs:
            df = pd.read_csv(
                f"{result_dir}/{time_shift}_60_320/model_evaluation/evaluation_output_constrained.txt", index_col=False
            )
        ...
        plot_df = main_df[["x", "y", metric]]
        # Create pivot table with mean values
        agg_xl = plot_df.groupby(["x", "y"])[metric].agg(["mean", "std", "count"]).reset_index()
        agg_xl["se"] = agg_xl["std"] / np.sqrt(agg_xl["count"])
        # Create pivot table with mean values
        pivot_mean = agg_xl.pivot(index="x", columns="y", values="mean")
        # Create pivot table with standard errors
        pivot_se = agg_xl.pivot(index="x", columns="y", values="se")
        # Add row for averages across validation sets (per test set)
        avg_row = agg_xl.groupby("x")["mean"].mean()
        pivot_mean.loc[:, 8] = avg_row
        pivot_se_avg = agg_xl.groupby("x")["se"].mean()
        ...
        if metric == "MSE":
            ...
            fs = 6
        elif metric == "Peak_Diff_percent":
            vmin, vmax = 0, 15
            metricl = "Peak Absolute Percentage Error"
            fs = 6
        elif metric == "ref_MSE":
            vmin, vmax = 0, 7
