...
st = read(f"{seis_dir}/9S.ILL11.EHZ.2019.172.mseed")
...
cache_path = Path(current_dir) / "fig7-cache/row1.npz"
...
# cache["labels"] = labels
scatter = ax.scatter(
    data["embedding_2d"][:, 0], data["embedding_2d"][:, 1], c=data["labels"], cmap=cmap, s=20, alpha=0.75
)
...
st = read(f"{seis_dir}/9S.ILL11.EHZ.2019.196.mseed")
...
stt[0].data = stt[0].data
timestamps = np.array([t for t in st[0].times("timestamp")])[5 * 60 * 100 :: 5 * 100]
cache_path = Path(current_dir) / "fig7-cache/row2.npz"
data = np.load(cache_path)
...
plt.tight_layout()
png_path = Path(current_dir) / "Figure-7.png"
