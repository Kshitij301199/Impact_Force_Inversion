...
def main(test_julday:int, val_julday:int, time_shift_minutes:int|str, smoothing:int,station:str, interval_seconds:int, config_option:str, task:str):
    ...
    if task == "abalation_study_1":
        ...
        image_dir = f"{paths['BASE_DIR']}/{task}/{time_shift_minutes}_{smoothing}/test_results/xlstm/{test_julday}"
        ...
        image_dir = f"{paths['BASE_DIR']}/{task}_{data_params['time_window']}_{data_params['fmin']}_{data_params['fmax']}/{time_shift_minutes}_{smoothing}/test_results/xlstm/{config_option}/{interval_seconds}"
