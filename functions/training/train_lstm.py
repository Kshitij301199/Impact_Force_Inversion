...
def main(
    ...
    if task.startswith("lr_test"):
        config_path = project_root / "config" / "lr_test" / f"lstm_{config_option}_{interval_seconds}sec_config.json"
    else:
    ...
    model = LSTMRegressor_v2(**model_params)
    summary(
        model=model,
    ...
    lr = args.lr if args.lr is not None else 1e-4  # keep old default as fallback only
    ...
    df.to_csv(f"{save_dir}/LSTM_t{test_julday}_v{val_julday}_val.csv", index=False)
    ...
    evaluate_model(
        model_type=f"LSTM,{config_option}",
        test_id=test_id,
        val_id=val_id,
        interval_seconds=interval_seconds,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", type=str, default="comparison_baseline", help="name of the task corresponding to parameter directory"
    )
    ...
    parser.add_argument("--grad_clip", type=float, default=2.0, help="max grad norm, 0 disables clipping")
