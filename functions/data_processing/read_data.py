...
def load_data(julday_list:list, station:str, year:int=2019, trim:bool=True, abs:bool=True) -> np.array:
    total_data = None
    for julday in julday_list:
        st = load_seismic_data(julday= julday, station= station, year=year, trim= trim)
        data = st[0].data[1:]
...
def load_seismic_data(julday:str|int, station:str, raw:bool=False, 
    ...
    if not raw:
        if type(julday) is int:
            try:
                st = read(f"{paths['BASE_DIR']}/{paths['DATA_DIR']}/{year}/{station}/EHZ/9S.{station}.EHZ.{year}.{julday}.mseed")
            except FileNotFoundError:
                st = read(f"{paths['LOCAL_BASE_DIR']}/{paths['DATA_DIR']}/{year}/{station}/EHZ/9S.{station}.EHZ.{year}.{julday}.mseed")
        ...
        if trim:
            ...
            if julday == 161:
                st.trim(starttime=trim_df.iloc[0,0] - (time_window * 120), endtime=trim_df.iloc[1,1])
            else:
                trim_df = trim_df[trim_df['Julday'] == julday]
                st.trim(starttime=trim_df.iloc[0,0] - (time_window * 120), endtime=trim_df.iloc[0,1] + (time_window * 120))
        return st
    else:
...
def load_label(date_list: list, station: str, interval_seconds: int, time_shift_minutes, trim:bool = True, smoothing: int | None = 30, divide_by: int | None = 350) -> pd.DataFrame:
    time_window = data_params['time_window']
    ...
    for i, date in enumerate(date_list):
        ...
        if i == 0:
            if trim:
                ...
                target_start_time = present_trim.iloc[0, 0] - (time_window * 60)  # Offset by 5/10 minutes
                if julday == 161:
                    # Special case for julday 161, use the second row for the end time
                    target_end_time = present_trim.iloc[1, 1] 
                else:
                    target_end_time = present_trim.iloc[0, 1] + (time_window * 120)
            else:
                target_start_time = UTCDateTime(f"{date}") + (time_window * 60)  # Offset by 5/10 minutes
                target_end_time = None
        else:
            if trim:
                # Use the second row of trim_df for subsequent dates
                target_start_time = present_trim.iloc[0, 0] - (time_window * 120)
                if julday == 161:
                    # Special case for julday 161, use the second row for the end time
                    target_end_time = present_trim.iloc[1, 1]
                else:
