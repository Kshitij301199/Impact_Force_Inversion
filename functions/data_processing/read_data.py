...
def load_data_test(julday_list:list, station:str, year:int=2019, abs:bool=True) -> np.array:
    total_data = None
    total_times = None
...
def load_seismic_data_test(julday:int|str|list, station:str, 
    ...
    if type(julday) is int or type(julday) is str:
        try:
            st = read(f"{paths['BASE_DIR']}/{paths['DATA_DIR']}/{year}/{station}/{component}/{network}.{station}.{component}.{year}.{str(julday).zfill(3)}.mseed")
        except FileNotFoundError:
            st = read(f"{paths['LOCAL_BASE_DIR']}/{paths['DATA_DIR']}/{year}/{station}/{component}/{network}.{station}.{component}.{year}.{str(julday).zfill(3)}.mseed")
        st[0].data = st[0].data * scaling
    elif type(julday) is list:
        st = Stream()
        for j in julday:
            try:
                st += read(f"{paths['BASE_DIR']}/{paths['DATA_DIR']}/{year}/{station}/{component}/{network}.{station}.{component}.{year}.{str(j).zfill(3)}.mseed")
            except:
                st += read(f"{paths['LOCAL_BASE_DIR']}/{paths['DATA_DIR']}/{year}/{station}/{component}/{network}.{station}.{component}.{year}.{str(j).zfill(3)}.mseed")
        st.merge(method=1, fill_value='latest', interpolation_samples=0)
        st[0].data = st[0].data * scaling
    else:
        print(f"Wrong julday type : {type(julday)}")
