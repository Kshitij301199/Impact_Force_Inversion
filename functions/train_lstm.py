...
def main(test_julday:int, val_julday:int, station:str, interval_seconds:int, config_option:str, task:str):
    ...
    image_dir = f"{paths['BASE_DIR']}/{task}/test_results_lstm/{config_option}/{interval_seconds}"
    ...
    evaluate_model(f"xLSTM,{config_option}", test_julday, val_julday, interval_seconds, np.concatenate(target_out), np.concatenate(pred_out))
