...
def clean_fv_max_outliers(target, data_col="Fv [kN]"):
    """
    Cap erroneous 'Fv max' values.

    Some rows have an 'Fv max' that is unrealistically high compared to the
    corresponding force value. Any 'Fv max' that is more than double the
    force value in `data_col` is replaced with 2x that force value.
    """
    if "Fv max" in target.columns:
        mask = (target["Fv max"] - target[data_col]) / target[data_col] > 1
        target.loc[mask, "Fv max"] = target.loc[mask, data_col] * 1.1
    return target
...
def main(time_shift_minutes, from_velocity: bool = False, avg_peak_shift: bool = False):
    ...
    if not from_velocity:  # CONSTANT TIME SHIFT BETWEEN SEISMIC AND IF PEAK
        ...
        data1["Time"] = data1.iloc[:, -1].apply(UTCDateTime) - (8.5 * 60) + (time_shift_minutes * 60)
        ...
        merged_data["Fv [kN]"] = merged_data["Fv [kN]"].apply(
            fill_missing_from_Gaussian, mean=min_Fv, std=sigma * min_Fv_std
        )
        merged_data = merged_data.fillna(value={"Fv std": min_Fv_std, "Fv min": 0, "Fv max": 5})
        merged_data = clean_fv_max_outliers(merged_data)
        merged_data[merged_data["Time"].between(UTCDateTime("2019-06-10"), UTCDateTime("2019-06-11"))].to_csv(
            f"{output_dir}/2019-06-10.csv", index=False
        )
        merged_data[merged_data["Time"].between(UTCDateTime("2019-06-11"), UTCDateTime("2019-06-12"))].to_csv(
            f"{output_dir}/2019-06-11.csv", index=False
        )
        ...
        merged_data = merged_data.fillna(value={"Fv std": min_Fv_std, "Fv min": 0, "Fv max": 5})
        merged_data = clean_fv_max_outliers(merged_data)
        merged_data[merged_data["Time"].between(UTCDateTime("2019-07-01"), UTCDateTime("2019-07-02"))].to_csv(
            f"{output_dir}/2019-07-01.csv", index=False
        )
        merged_data[merged_data["Time"].between(UTCDateTime("2019-07-02"), UTCDateTime("2019-07-03"))].to_csv(
            f"{output_dir}/2019-07-02.csv", index=False
        )
        merged_data[merged_data["Time"].between(UTCDateTime("2019-07-03"), UTCDateTime("2019-07-04"))].to_csv(
            f"{output_dir}/2019-07-03.csv", index=False
        )
        ...
        peak_diff_values = [1.1, 9.5, 1.3, -2.7, 2.5]

        for date_start, date_end, shift, peak_diff in tqdm(
            zip(date_starts, date_ends, shift_values, peak_diff_values), total=5
        ):
        ...
        data1["Time"] = (
            data1.iloc[:, -1].apply(UTCDateTime) - (8.5 * 60) + int(distance / temp.iloc[0, 4])
        )  # - peak_to_peak difference + time shift by velocity
        data2["Time"] = (
            data2.iloc[:, -1].apply(UTCDateTime) - (2.1 * 60) + int(distance / temp.iloc[1, 4])
        )  # - peak_to_peak difference + time shift by velocity
        data = pd.concat([data1, data2])
        complete_data = pd.DataFrame(columns=["Time"])
        start_time = UTCDateTime("2019-06-10T00:00:00")
        ...
        data1["Time"] = (
            data1.iloc[:, -1].apply(UTCDateTime) - (8.5 * 60) + int(distance / avg_velocity)
        )  # - peak_to_peak difference + time shift by velocity
        data2["Time"] = (
            data2.iloc[:, -1].apply(UTCDateTime) - (2.1 * 60) + int(distance / avg_velocity)
        )  # - peak_to_peak difference + time shift by velocity
        data = pd.concat([data1, data2])
        complete_data = pd.DataFrame(columns=["Time"])
        start_time = UTCDateTime("2019-06-10T00:00:00")
