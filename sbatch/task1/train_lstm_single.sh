...
#SBATCH --array=1-126%6           # job array id, adjusted for the total number of commands (7 test days * 6 validation days * 3 time shift modes * 1 interval * 1 smoothing * 1 hyp option)
...
num_phases=${#phases[@]}
num_combinations=$(( num_val_ids * num_intervals * num_smoothings * num_hyp_options * num_phases ))
...
val_id_idx=$(( $remaining_idx / (num_intervals * num_smoothings * num_hyp_options * num_phases) ))
remaining_idx=$(( $remaining_idx % (num_intervals * num_smoothings * num_hyp_options * num_phases) ))

time_shift_idx=$(( $remaining_idx / (num_intervals * num_smoothings * num_hyp_options) ))
...
# Get the current interval, smoothing, hyp option, and time shift
interval=${intervals[$interval_idx]}
smoothing=${smoothings[$smoothing_idx]}
hyp_option=${hyp_options[$hyp_option_idx]}
time_shift_mins=${phases[$time_shift_idx]}
...
echo "Time Shift: $time_shift_mins"
...
srun --gres=gpu:A40:1 --unbuffered python /storage/vast-gfz-hpc-01/home/kshitkar/Impact_Force_Inversion/functions/training/train_lstm.py \
    ...
    --time_shift_mins "$time_shift_mins" \
    --interval "$interval" \
    --station "ILL11" \
    --task "task1" \
    --smoothing "$smoothing" \
