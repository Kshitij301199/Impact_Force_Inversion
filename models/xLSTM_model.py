...
class xLSTMRegressor_v2(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.fc(x)  # (batch_size, 1)
        return F.softplus(output)  # raw output, optionally apply softplus externally if needed
...
class xLSTMRegressor_v4(nn.Module):
    """xLSTMRegressor with BatchNorm1d after embedding and before xLSTM layers."""
    def __init__(self,
                 input_size,
                 conv1d_kernel_size=4, 
                 qkv_proj_blocksize=4, 
                 ...
                 slstm_at=[1],
                 num_reps: int = 1):
        super(xLSTMRegressor_v4, self).__init__()
        
        self.embedding = nn.Linear(input_size, embedding_dim)
        self.batchnorm = nn.BatchNorm1d(context_length)  # context_length = sequence length (e.g., 10)
        backend = "vanilla"
    ...
    def forward(self, x):
        ...
        # BatchNorm1d expects (batch_size, seq_len, embedding_dim) -> (batch_size, seq_len, embedding_dim)
        # But BatchNorm1d normalizes over the second dimension, so we need to swap axes
        # x = x.permute(0, 2, 1)  # (batch, embedding_dim, seq_len)
        x = self.batchnorm(x)
        # x = x.permute(0, 2, 1)  # (batch, seq_len, embedding_dim)

        for xlstm in self.xlstm_layers:
            x = xlstm(x)  # (batch_size, seq_len, embedding_dim + 1)
